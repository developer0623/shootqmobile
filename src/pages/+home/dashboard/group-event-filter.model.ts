...
export interface EventGroupFilterParams {
  ...
  event_type?: string;
  job__job_workers__worker?: string;
  job__job_workers__worker__isnull?: boolean;
  booked?: boolean;
  lead?: boolean;
}
...
export class EventGroupFilter {
  ...
  toQueryParams() {
    ...
    if (this.params.job__job_workers__worker)
      result['job__job_workers__worker'] = this.params.job__job_workers__worker;

    let booleanFields = ['job__job_workers__worker__isnull', 'event_type__isnull', 'booked', 'lead'];
